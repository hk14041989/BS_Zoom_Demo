...
namespace BS_Zoom_Demo.Tests
{
    public abstract class BS_Zoom_DemoTestBase : AbpIntegratedTestBase<BS_Zoom_DemoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected BS_Zoom_DemoTestBase()
        {
        ...
        protected void UsingDbContext(Action<BS_Zoom_DemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BS_Zoom_DemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BS_Zoom_DemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BS_Zoom_DemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BS_Zoom_DemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BS_Zoom_DemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BS_Zoom_DemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BS_Zoom_DemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BS_Zoom_DemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BS_Zoom_DemoDbContext, Task<T>> func)
        {
            T result;
